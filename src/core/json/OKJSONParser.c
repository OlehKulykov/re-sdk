...
uint32_t OKJSONParserIncCapacity(OKJSONParserStruct * p)
{
	const int newCapacity = p->capacity + 16;
										...
										PARSED_OBJECT * resString)
{
	...
	if (newBuffer) 
	{
        ...
        PARSED_OBJECT newString = (PARSED_OBJECT)p->callbacks->_createStringWithUTF8((const char*)startNewBuff, (int)((const char*)newBuffer - (const char*)startNewBuff), p->callbacks->userData);
...
void OKJSONParserParseString(OKJSONParserStruct * p, PARSED_OBJECT * resString)
{
	...
	if (isHasReplacement) OKJSONParserParseReplacementString(p, start, (int)(data - start), resString);
    else *resString = (PARSED_OBJECT)p->callbacks->_createStringWithUTF8((const char*)start, (int)(data - start), p->callbacks->userData);
