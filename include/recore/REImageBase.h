...
class __RE_PUBLIC_CLASS_API__ REImageBase
{
protected:
	REBuffer _buffer;
	REUInt16 _width;
...
public:
	...
	REUByte * imageDataAtCoords(const REUInt32 x, const REUInt32 y) const;
	...
	/// Returns row stride which is bytes per pixel of the format multiplyed to width.
	static REUInt32 rowStride(const REImagePixelFormat format, const REUInt32 width);
