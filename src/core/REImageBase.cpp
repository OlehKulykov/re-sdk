...
const REUInt32 REImageBase::rowStride() const
{
	return REImageBase::rowStride((REImagePixelFormat)_format, (REUInt32)_width);
}

REUByte * REImageBase::imageDataAtCoords(const REUInt32 x, const REUInt32 y) const
{
	if (_format && (x <= _width) && (y <= _height)) 
	{
...
REUInt32 REImageBase::rowStride(const REImagePixelFormat format, const REUInt32 width)
{
	switch (format) 
	{
		case REImagePixelFormatR8G8B8:
