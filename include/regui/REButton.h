...
protected:
	RETexture * _pressedTexture;
	RETexture * _disabledTexture;
...
public:
	...
	virtual void setTextureForState(RETexture * newTexture, REButton::StateType state);
	
	/// Retuns pointer to texture for state. If no texture assigned NULL wil return.
	virtual RETexture * textureForState(REButton::StateType state);
	...
	RETypedPtr serializeToDictionary() const;
	
	void deserializeWithDictionary(const RETypedPtr & dictionary);
