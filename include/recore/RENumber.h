...
public:
	...
	{
		if (this->isReal()) return [NSNumber numberWithDouble:this->float64Value()];
		else
		{
			if (this->isSigned()) return [NSNumber numberWithLongLong:this->int64Value()];
			else return [NSNumber numberWithUnsignedLongLong:this->uint64Value()];
		}
	}
	void setNSNumber(NSNumber * num)
	{
		if (num) 
		{
			*this = RENumber((const char *)[[num stringValue] UTF8String]);
		}
	}
#endif	
};
