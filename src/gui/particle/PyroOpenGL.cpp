...
{
	GLuint texturesArray[1] = { 0 };
	GLuint * texturesPointer = (GLuint *)&texturesArray;
	glGenTextures(1, texturesPointer);
	m_glTexture = texturesArray[0];
...
PyroParticles::CTexture_OGL::~CTexture_OGL()
{
	if ( m_glTexture != 0 )
	{
		GLuint texturesArray[1] = { m_glTexture };
		const GLuint * texturesPointer = (const GLuint *)&texturesArray;
		glDeleteTextures(1, texturesPointer);
	}
}
...
PyroResult PyroParticles::CGraphics_OGL::RenderQuads(
													 ...
													 PyroDWord Flags)
{
	return this->RenderQuads2(_pVertexBuffer, nFirst, nQuads, Flags);
...
#ifdef __RE_RENDER_DEVICE_OPENGL__	
	RERenderDeviceOpenGL * renderDeviceGL = RERenderDevice::GetDefaultDeviceOpenGL();
	renderDeviceGL->SetGLBlendFunction(glSrcBlend, glDestBlend);
#endif	
...
void PyroParticles::CGraphics_OGL::CreateQuadIndexBuffer(int nQuads)
{
	...
	UInt16 *pDest = (UInt16 *)pIndexBuffer->GetBuffer();
	UInt16 nVertex = 0;
