...
{
	return REAnimationController::defaultController()->startSetupNewAnimation(customDataOrNull);
}

REBOOL REAnimation::isSetuping()
{
	return (REAnimationController::defaultController()->lastInfo() != NULL);
}
...
{
	return REAnimationController::defaultController()->addFloatParam(animatedObject,
...
{
	REEditableAnimationInfo * info = REAnimationController::defaultController()->lastInfo();
...
{
	return REAnimationController::defaultController()->executeLastAnimation();
}

void REAnimation::pauseAllAnimations()
{
	REAnimationController::defaultController()->pause();
}

void REAnimation::continueAllAnimations()
{
	REAnimationController::defaultController()->Continue();
}
...
{
	return REAnimationController::defaultController()->stopAllAnimationForView(animatedObject, stopType, isNeedCallDelegate);
}
...
const REFloat32 REAnimation::getProgress(REObject * animatedObject)
{
	if (animatedObject) 
	{
		return REAnimationController::defaultController()->getProgress(animatedObject);
	}
	return -1.0f;
}
